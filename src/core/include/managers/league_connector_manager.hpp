...
namespace league_constants {
    ...
    inline uint64_t hash_constant(const std::string& value) {
        return ru_hash_string_64(value.c_str());
    }

    inline constexpr uint64_t hash_constant(const char* value) {
        return ru_hash_string_64(value);
    }
...
public:
    ...
    void add_connect_handler(const ru_function<void>& callback);
    void add_disconnect_handler(const ru_function<void>& callback);
